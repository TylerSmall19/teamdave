/**
 * @license jCanvas Handles v16.10.17
...
function addRectHandle( $canvas, parent, px, py ) {
  ...
  handle = $.extend( {
    ...
    fromCenter: true,
    dragstart: function ( layer ) {
      layer._restictedX = layer.x;
      layer._restictedY = layer.y;
      $( this ).triggerLayerEvent( layer._parent, 'handlestart' );
    },
    ...
    drag: function ( layer ) {
      ...
      if ( !parent.resizeFromCenter ) {
        ...
        if ( layer.restrictDragToAxis !== 'y' || parent.constrainProportions || !parent.resizeFromCenter ) {
          parent.width += layer.dx * layer._px * 2;
        }
        if ( layer.restrictDragToAxis !== 'x' || parent.constrainProportions || !parent.resizeFromCenter ) {
...
function updateRectHandles( parent ) {
  ...
  if ( parent._handles ) {
    ...
    for ( h = 0; h < parent._handles.length; h += 1 ) {
      handle = parent._handles[h];
      if ( handle.restrictDragToAxis !== 'y' || parent.constrainProportions || !parent.resizeFromCenter ) {
        handle.x = parent.x + ( parent.width / 2 * handle._px + ( ( parent.fromCenter ) ? 0 : parent.width / 2 ) );
      }
      if ( handle.restrictDragToAxis !== 'x' || parent.constrainProportions || !parent.resizeFromCenter ) {
